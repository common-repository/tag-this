...
class TagThis
{
    ...
    private function addLink($id)//write the actual tag to the database
    {
        global $wpdb;
    ...
    private function removelink($id)//remove the actual tag
    {
        global $wpdb;
        $postid=$wpdb->get_var("SELECT postid from $wpdb->tagthis WHERE id=$id");
    ...
    function addTag($postid, $tag1)
    {
        ...
        if($status==1)
        {
            ...
            if($numtags>=get_option("tt_nfrontpage"))
            {                                                                                            
                $this->addLink($id);                   
